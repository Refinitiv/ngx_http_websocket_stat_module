...
void insert_occurance(const template_variable *var, size_t pos, compiled_template *template_cmlp)
{
...
static void find_variables(const char *template, compiled_template *template_cmlp)
{
   int i = 0; 
...
size_t estimate_size(compiled_template *template_cmpl)
{
   ...
   {
      variable_occurance * occ = ((variable_occurance **)template_cmpl->variable_occurances->elts)[i];
      size_dif += occ->variable->len - 
...
void _compile_template(compiled_template *template_cmpl)
{
   ...
   template_cmpl->compiled_template_str = ngx_palloc(template_cmpl->pool, size + 1);
...
compiled_template *
compile_template(const char *template,
                 template_variable* variables,
                 ngx_pool_t * pool)
{
   compiled_template * templ = ngx_palloc(pool, sizeof(compiled_template));
   templ->variable_occurances = ngx_array_create(pool, 10, sizeof(variable_occurance*));
