...
char parse_message(u_char** buffer, size_t *size, ngx_frame_counter_t *frame_counter)
{
  while(*size > 0)
   {
     switch (frame_counter->stage)
     {
        ...
        case MASK:
           ...
           break;
        case PAYLOAD:
           if(*size >= (u_int)(frame_counter->current_payload_size - frame_counter->bytes_consumed))
           {
              
              move_buffer(buffer, size, frame_counter->current_payload_size - frame_counter->bytes_consumed);
