...
static char *
ngx_http_websocket_stat_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child);
...
ssize_t
(* orig_recv)(ngx_connection_t *c, u_char *buf, size_t size);
...
static ngx_int_t ngx_http_ws_stat_create_request(ngx_http_request_t *r)
{
    ...
    u_int request_len = 0;
    request_len = method.len + 1 + sizeof(ngx_http_proxy_version) - 1
                  + sizeof(CRLF) - 1;
    ...
    *buffer->last++ = ' ';
    ...
    while(i != 0){
        if (header[i].key.data && header[i].value.data)
        {
            buffer->last = ngx_copy(buffer->last, header[i].key.data, header[i].key.len);

            *buffer->last++ = ':'; *buffer->last++ = ' ';

            buffer->last = ngx_copy(buffer->last, header[i].value.data,
                               header[i].value.len);
